...
set GRADLE_ZIP_PATH=%GRADLE_DIR%\%GRADLE_ZIP%
powershell -Command "& { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%GRADLE_URL%' -OutFile '%GRADLE_ZIP_PATH%' }"
if %ERRORLEVEL% neq 0 (
...
echo [*] Extracting...
powershell -Command "& { Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory('%GRADLE_ZIP_PATH%', '%GRADLE_DIR%') }"
if %ERRORLEVEL% neq 0 (
